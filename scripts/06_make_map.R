...
ecotype_hc <- ifelse(site_info$ecotype=="Semelparous", "black", "black")
...
# Plot
png("figures/figure1.png", width=plot_width, height=plot_width*plot_ratio, units="in", res=1500)
par(bty="n", bg=water_color, mar=c(0,0,0,0), oma=c(0,0,0,0), family="Verdana")
...
text(site_info, labels=site_info$site_code, 
     ...
     pos=c(2, 3, 1, 4, 2, 4), cex=2, font=2)
addscalebar(plotunit="latlon", unit="metric")
par(usr=c(min(inset_poly$long)-1, min(inset_poly$long)+40, max(inset_poly$lat)-50, max(inset_poly$lat)+1))
